...
def angle_to_north(gps1, gps2):
    ...
    x = cos(radians(gps1["lat"])) * sin(radians(gps2["lat"])) - sin(radians(gps1["lat"])) * cos(
        radians(gps2["lat"])) * cos(dlon)
...
def predict(data):
    ...
    file_path = os.path.join(module_dir, 'rf.pkl')
...
def solve_label():
    global meta_counter
    key = datetime.strftime(log_last["start_time"], "%Y/%m/%d|%H:%M:%S|") + datetime.strftime(log_last["end_time"],
                                                                                              "%H:%M:%S")
